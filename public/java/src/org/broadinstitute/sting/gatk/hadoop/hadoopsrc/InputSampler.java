...
public class InputSampler<K, V> extends Configured implements Tool {
	...
	public static class SplitSampler<K, V> implements Sampler<K, V> {
		...
		public K[] getSample(InputFormat<K, V> inf, Job job)
			...
			for (int i = 0; i < splitsToSample; ++i) {
				...
				RecordReader<K, V> reader = inf.createRecordReader(splits
						.get(i), samplingContext);
				...
				while (reader.nextKeyValue()) {
					if (r.nextDouble() <= freq) {
						if (samples.size() < numSamples) {
							samples.add(ReflectionUtils.copy(job
									.getConfiguration(),
									reader.getCurrentKey(), null));
						} else {
							...
							if (ind != numSamples) {
								samples.set(ind, ReflectionUtils.copy(job
	...
	public static <K, V> void writePartitionFile(Job job, Sampler<K, V> sampler)
		...
		final InputFormat inf = ReflectionUtils.newInstance(job
